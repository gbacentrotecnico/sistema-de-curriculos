      ...
      <div className="max-w-xl mx-auto px-4 py-16 text-center animate-in fade-in zoom-in duration-500">
        <div className="bg-white dark:bg-gray-900 p-8 rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-800 flex flex-col items-center">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-8 text-lg">
            ...
            A equipe da <span className="font-bold text-yellow-500">Grupo Abucci</span> analisará seu perfil.
          </p>
          <button 
            onClick={() => setSubmitted(false)}
            className="bg-yellow-500 hover:bg-yellow-600 text-zinc-900 px-8 py-3 rounded-xl font-bold transition-all shadow-lg active:scale-95"
          >
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      <div className="mb-10 text-center">
        <span className="bg-yellow-100 dark:bg-yellow-800/40 text-yellow-600 dark:text-yellow-300 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] mb-4 inline-block border border-yellow-200 dark:border-yellow-700">
          Carreiras
        </span>
        <h1 className="text-4xl md:text-5xl font-black text-gray-900 dark:text-white mb-4 tracking-tighter leading-none">
          CUIDANDO DE VIDAS COM <span className="text-yellow-500">EXCELÊNCIA</span>
        </h1>
        <p className="text-gray-600 dark:text-gray-400 text-lg max-w-lg mx-auto leading-tight font-medium">
          Envie seus dados para nossa central e faça parte da equipe Grupo Abucci.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-800 p-8 md:p-12 space-y-8 relative overflow-hidden">
        {loading && (
          <div className="absolute inset-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm z-10 flex flex-col items-center justify-center space-y-4">
             <div className="relative">
                ...
                <HeartPulse className="h-5 w-5 text-yellow-500 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
             </div>
             <p className="font-bold uppercase text-yellow-500 animate-pulse text-sm">{uploadStep}</p>
          </div>
        )}
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input 
                required
                type="email"
                className="w-full pl-12 pr-4 py-4 rounded-2xl border border-gray-200 dark:border-gray-800 dark:bg-gray-800 focus:ring-4 focus:ring-yellow-500/10 focus:border-yellow-500 transition-all outline-none font-medium text-gray-800 dark:text-white"
        ...
        <div className="space-y-2">
          ...
          <div className="mt-1 flex justify-center px-6 pt-12 pb-12 border-2 border-gray-200 dark:border-gray-800 border-dashed rounded-3xl hover:border-yellow-500 dark:hover:border-yellow-400 transition-all cursor-pointer group relative bg-gray-50/50 dark:bg-gray-800/30">
            <input 
              required
              type="file" 
              accept=".pdf,.doc,.docx"
        ...
        <button 
          disabled={loading}
          type="submit"
          className="w-full bg-yellow-500 hover:bg-yellow-600 text-zinc-900 py-5 rounded-2xl font-bold text-xl uppercase shadow-xl hover:shadow-yellow-500/30 transition-all flex items-center justify-center gap-3 disabled:opacity-50 active:scale-[0.98]"
        >
          {loading ? (
